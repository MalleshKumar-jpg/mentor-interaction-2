 ...
 User* find_user(char* username) {
     ...
     while (user != NULL) {
         ...
         user = user->next; //we iterate through linked list , which is used for collision handling
     }
...
//adds the given user struct to hash table
 ...
 bool delete_task(User* mentee, int task_index) { //task_index is input based on the task user has chosen to delete (tasks are displayed in the website in the same order as linked list)
    ...
    // Special case: deleting the first task (index 0)
    if (task_index == 0) {
 ...
 bool add_meeting_note(User* mentee, char* date, char* summary) {
    ...
    mentor->data.mentor_data.mentees_bst_root = remove_bst_node(
        mentor->data.mentor_data.mentees_bst_root, mentee);
    mentor->data.mentor_data.mentees_bst_root = insert_bst_node(
        mentor->data.mentor_data.mentees_bst_root, mentee);
 ...
 bool is_valid_email(char* email) {
     // Basic email validation - contains @ and a period
     char* at_sign = strchr(email, '@');
     if (!at_sign) {
         return false;
     }
     
     char* period = strchr(at_sign, '.');
     if (!period) {
         return false;
     }
 ...
 bool is_valid_digital_id(char* digital_id) {
     // Digital ID should be 7 digits
     int len = strlen(digital_id);
     if (len != 7) {
         return false;
     }
     
     for (int i = 0; i < len; i++) {
         if (digital_id[i] < '0' || digital_id[i] > '9') {
             return false;
         }
     }
 ...
 bool is_valid_registration_number(char* reg_number) {
     // Registration number should be 13 digits
 ...
 User* register_mentor(char* username, char* password, char* name, char* email, 
                      char* phone, char* department) {
 ...
 User* register_mentee(char* username, char* password, char* name, char* email, 
                      char* phone, char* department, int year, char* digital_id, 
                      char* registration_number, char* parent_contact, User* mentor) {
     if (!is_valid_email(email) || !is_valid_phone(phone) || 
         !is_valid_digital_id(digital_id) || !is_valid_registration_number(registration_number)) {
         return NULL;
     }
 ...
 // Update functions
 bool update_mentee_info(User* mentee, char* name, char* email, char* phone, 
                        char* department, int year, char* digital_id, 
                        char* registration_number, char* parent_contact) {
     if (mentee->role != MENTEE) {
         return false;
     }
 ...
 void generate_sample_data() {
     ...
     User* mentor1 = register_mentor("mentor1", "password", "John Smith", "john@example.com", 
                                   "1234567890", "Computer Science");
     User* mentor2 = register_mentor("mentor2", "password", "Jane Doe", "jane@example.com", 
                                   "9876543210", "Mathematics");
     
     // Create sample mentees
     User* mentee1 = register_mentee("mentee1", "password", "Alice Johnson", "alice@example.com", 
                                   "1112223333", "Computer Science", 2, "1234567", "1234567890123", 
                                   "parent1@example.com", mentor1);
     User* mentee2 = register_mentee("mentee2", "password", "Bob Brown", "bob@example.com", 
                                   "4445556666", "Mathematics", 3, "2345678", "2345678901234", 
                                   "parent2@example.com", mentor1);
    User* mentee3 = register_mentee("mentee3", "password", "Carol Wilson", "carol@example.com", 
                                  "7778889999", "Computer Science", 1, "3456789", "3456789012345", 
                                  "parent3@example.com", mentor2);
    
    // Add tasks to mentees
    add_task(mentee1, "Complete programming assignment", "15-05-2025");
    add_task(mentee1, "Prepare for midterm exam", "20-05-2025");
    add_task(mentee2, "Submit research proposal", "10-05-2025");
...
void initialize_system() {
    ...
    // If no users were loaded, generate sample data
...
User* api_register_mentor(char* username, char* password, char* name, char* email, 
                         char* phone, char* department) {
...
User* api_register_mentee(char* username, char* password, char* name, char* email, 
                         char* phone, char* department, int year, char* digital_id, 
                         char* registration_number, char* parent_contact, char* mentor_username) {
...
bool api_update_mentee_info(char* username, char* name, char* email, char* phone, 
                          char* department, int year, char* digital_id, 
                          char* registration_number, char* parent_contact) {
    ...
    bool success = update_mentee_info(mentee, name, email, phone, department, year, 
                                    digital_id, registration_number, parent_contact);
    if (success) {
        save_users_to_file();
    }
    return success;
